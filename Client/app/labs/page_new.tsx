...
export default function MyLabsPage() {
  ...
  const handleReschedule = async (booking: LabBooking) => {
    try {
      // Simplified inline reschedule for now
      const newDate = prompt('Enter new date (YYYY-MM-DD):');
      const newTime = prompt('Enter new time (HH:MM):');
      if (newDate && newTime) {
        await labService.reschedule(booking.id, newDate, newTime);
        await refresh();
      }
    } catch (e: any) {
  ...
  const handleCancel = async (booking: LabBooking) => {
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={() => router.push('/patient-dashboard/lab-tests')} className="bg-[#4DBCC4] hover:bg-[#4DBCC4]/90 dark:bg-[#2a5f6b] dark:hover:bg-[#2a5f6b]/90 text-white">
        ...
        {filtered.length === 0 ? (
          <Card className="text-center py-12">
            <CardContent>
              ...
              <Button onClick={() => router.push('/patient-dashboard/lab-tests')} className="bg-[#4DBCC4] hover:bg-[#4DBCC4]/90 dark:bg-[#2a5f6b] dark:hover:bg-[#2a5f6b]/90 text-white">{t('bookTest') || 'Book Test'}</Button>
            </CardContent>
          </Card>
        ) : (
